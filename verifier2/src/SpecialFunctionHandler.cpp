...
void SpecialFunctionHandler::handlePrintExpr(ExecutionState &state,
  ...
  std::string prefix = msg_str.substr(0,10);
  if (prefix == "[State ID]"){
    std::string remaind = msg_str.substr(10);
    llvm::errs() << "[~State ID]" << "[" << state.getID() << "] "<< remaind << arguments[1]<<"\n";
  }else if (prefix == "[PathCond]"){
    for (auto it = state.constraints.begin(); it != state.constraints.end(); it++){
      llvm::errs() << "[~State ID]" << "[" << state.getID() << "] ";
      llvm::errs() << "[PathCond]";
      it->get()->print(llvm::errs()); llvm::errs()<<"\n";
    }
  }else{
